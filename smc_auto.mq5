...
input group "=== SMC Trading Capital Management ==="
input int    InpMagicNumber       = 20260606; // Magic Number quản lý lệnh
input double InpMaxRiskPercent    = 2.0;      // Tỷ lệ sụt giảm vốn tối đa (% Balance, Tối đa 50%)
...
//+------------------------------------------------------------------+
//| Hàm tính toán khối lượng Lot size dựa trên mức rủi ro tiền tệ    |
//+------------------------------------------------------------------+
double CalculateLotSize(double entryPrice, double slPrice, double riskAmount)
{
   ...
   if(tickSize <= 0) tickSize = _Point; // Đảm bảo an toàn nếu tick size không hợp lệ
   if(tickValue <= 0) 
   {
      // Nếu không lấy được tickValue, giả định tài khoản trùng tiền tệ giao dịch và tính thô
      tickValue = tickSize; 
   }
   
   // Tính số tiền rủi ro cho mỗi lotstep
   double moneyPerLotStep = (slDistance / tickSize) * tickValue * lotStep;
   if(moneyPerLotStep <= 0) return 0.0;
   
   double lots = (riskAmount / moneyPerLotStep) * lotStep;
   
   // Bo tròn theo Volume Step của Broker
   lots = MathFloor(lots / lotStep) * lotStep;
   
   // Giới hạn theo khối lượng tối thiểu và tối đa
   if(lots < minVol) return 0.0; // Khoảng cách SL quá rộng hoặc rủi ro quá nhỏ dẫn đến lot < minVol
...
//+------------------------------------------------------------------+
//| Kiểm tra tính hợp lệ của lệnh trước khi đặt (Margin và Lot size) |
//+------------------------------------------------------------------+
bool ValidateOrderExecution(ENUM_ORDER_TYPE orderType, double lots, double entryPrice)
{
   if(lots <= 0.0)
   {
      Print("SMC Trade Error: Khối lượng lot không hợp lệ hoặc quá nhỏ (lots = 0.0).");
      return false;
   }
   ...
   {
      Print("SMC Trade Error: Không thể tính toán Margin. Mã lỗi: ", GetLastError());
      return false;
   }
   ...
   {
      Print("SMC Trade Error: Không đủ Margin ký quỹ. Yêu cầu: ", margin, ", Khả dụng: ", freeMargin);
      return false;
   }
...
//+------------------------------------------------------------------+
//| Hàm chính thực hiện đặt lệnh Market và các lệnh Limit cấu hình   |
//| - direction: 1 = BUY, -1 = SELL                                  |
//| - swingHigh: Đỉnh cấu trúc (TP cho Buy, SL cho Sell)             |
//| - swingLow: Đáy cấu trúc (SL cho Buy, TP cho Sell)               |
//| - entryPrices: Mảng chứa các điểm vào lệnh (0.0 biểu thị Market) |
//+------------------------------------------------------------------+
void PlaceSMCOrders(int direction, double swingHigh, double swingLow, const double &entryPrices[])
{
   int nOrders = ArraySize(entryPrices);
   if(nOrders <= 0)
   {
      Print("SMC Trade Warning: Mảng điểm vào lệnh (entryPrices) rỗng.");
      return;
   }
   
   // 1. Quản lý vốn & Giới hạn rủi ro tối đa 50% Balance
   double riskPercent = InpMaxRiskPercent;
   if(riskPercent > 50.0)
   {
      Print("SMC Trade Warning: Tỷ lệ rủi ro vượt quá 50% số dư tài khoản. Tự động chuyển về 50.0%.");
      riskPercent = 50.0;
   }
   if(riskPercent <= 0)
   {
      Print("SMC Trade Error: Tỷ lệ rủi ro phải lớn hơn 0.");
      return;
   }
   
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   double totalRiskAmount = balance * (riskPercent / 100.0);
   
   // Chia đều rủi ro cho mỗi lệnh
   double riskPerOrder = totalRiskAmount / nOrders;
   
   // Thiết lập Stop Loss và Take Profit theo hướng lệnh
   double slPrice = (direction == 1) ? swingLow : swingHigh;
   double tpPrice = (direction == 1) ? swingHigh : swingLow;
   
   // Lấy giá thị trường hiện tại để kiểm tra
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   
   // Thiết lập Magic Number cho đối tượng giao dịch
   tradeSMC.SetExpertMagicNumber(InpMagicNumber);
   
   // Lặp qua từng điểm vào lệnh trong mảng
   for(int i = 0; i < nOrders; i++)
   {
      double rawEntry = entryPrices[i];
      double entryPrice = rawEntry;
      bool isMarketOrder = false;
      
      // Nếu giá trị vào lệnh bằng 0.0 hoặc rất gần 0.0, coi là lệnh Market
      if(rawEntry <= 0.0)
      {
      ...
      if(slDistance <= 0.0)
      {
         Print("SMC Trade Error: Điểm vào lệnh [", i, "] bằng điểm Stop Loss. Bỏ qua lệnh này.");
         continue;
      }
      
      // Tính toán Lot Size cho lệnh thứ i
      double lots = CalculateLotSize(entryPrice, slPrice, riskPerOrder);
      
      // Xác định loại lệnh đặt
      ...
      if(direction == 1) // BUY
      {
         ...
         {
            // Nếu giá Limit cao hơn hoặc bằng giá Ask hiện tại, chuyển thành lệnh Market Buy để khớp luôn
            if(entryPrice >= ask)
            {
               entryPrice = ask;
               orderType = ORDER_TYPE_BUY;
               isMarketOrder = true;
               Print("SMC Trade Info: Giá Buy Limit cao hơn giá Ask. Chuyển thành lệnh Market Buy.");
            }
            else
            {
      ...
      else // SELL
      {
         ...
         {
            // Nếu giá Limit thấp hơn hoặc bằng giá Bid hiện tại, chuyển thành lệnh Market Sell để khớp luôn
            if(entryPrice <= bid)
            {
               entryPrice = bid;
               orderType = ORDER_TYPE_SELL;
               isMarketOrder = true;
               Print("SMC Trade Info: Giá Sell Limit thấp hơn giá Bid. Chuyển thành lệnh Market Sell.");
            }
            else
            {
      ...
      // Kiểm tra tính hợp lệ của lệnh trước khi thực thi
      if(!ValidateOrderExecution(orderType, lots, entryPrice))
      {
         Print("SMC Trade Error: Lệnh thứ [", i, "] không hợp lệ (không đủ ký quỹ hoặc Lot size nằm ngoài phạm vi cho phép). Không đặt lệnh.");
         continue;
      }
      
      // Thực thi đặt lệnh
      bool orderSuccess = false;
      string comment = "SMC Order " + IntegerToString(i + 1) + "/" + IntegerToString(nOrders);
      
      if(direction == 1) // BUY
      {
         if(orderType == ORDER_TYPE_BUY)
         {
            orderSuccess = tradeSMC.Buy(lots, _Symbol, entryPrice, slPrice, tpPrice, comment);
         }
         else // ORDER_TYPE_BUY_LIMIT
         {
            orderSuccess = tradeSMC.BuyLimit(lots, entryPrice, _Symbol, slPrice, tpPrice, ORDER_TIME_GTC, 0, comment);
         }
      }
      else // SELL
      {
         ...
         {
            orderSuccess = tradeSMC.Sell(lots, _Symbol, entryPrice, slPrice, tpPrice, comment);
         }
         else // ORDER_TYPE_SELL_LIMIT
         {
            orderSuccess = tradeSMC.SellLimit(lots, entryPrice, _Symbol, slPrice, tpPrice, ORDER_TIME_GTC, 0, comment);
         }
      }
      ...
      if(orderSuccess)
      {
         Print("SMC Trade Success: Đã đặt thành công lệnh [", i + 1, "/", nOrders, 
               "] Loại: ", EnumToString(orderType), 
      ...
      {
         Print("SMC Trade Error: Không thể đặt lệnh [", i + 1, "/", nOrders, "]. Lỗi: ", tradeSMC.ResultRetcodeDescription());
      }
   }
}
