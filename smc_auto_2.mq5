   ...
   //    swing high
      if (bar3.high <= bar2.high && bar2.high >= bar1.high) { // tim thay dinh high
         ...
         if (tfData.iTrend == -1 && tfData.LastSwingInternal == 1 && bar2.high > tfData.intSHighs[0] 
            ...
            textInternalHigh += "\n"+"## High 6 iCHoCH --> Update: LastSwingInternal: -1, New intSHighs[0]: "+(string) bar2.high;
...
//      if (tfData.iFindTarget == 0 && tfData.iTarget == 0) {
//         Print(" -------------------------- Reset xoa sach ban do");
//      }
//      if (tfData.iTarget != 0 && tfData.iStoploss != 0) {
//         if (tfData.iFindTarget != 0) {
//            Print("================> "+((tfData.iFindTarget == 1)? "iBullish" : "iBearish")+"; iTarget = "+ (string) tfData.iTarget + " has time = "+(string) tfData.iTargetTime+" && iStoploss = "+ (string) tfData.iStoploss + " has time = "+ (string) tfData.iStoplossTime);
//         }
//         
//      }
      if(isComment) {
   ...
   void updatePointTopBot(TimeFrameData& tfData, MqlRates& bar1, MqlRates& bar2, MqlRates& bar3, bool isComment = false){
      ...
      if(tfData.sTrend == 1 && tfData.mTrend == 1) {
         ...
         if (  
            ...
            text += "\n Cap nhat: New arrPbHigh = arrTop[0] = "+ (string) tfData.arrTop[0];
      ...
      if(tfData.sTrend == -1 && tfData.mTrend == -1) {
         ...
         if (
            ...
            text += "\n Cap nhat: New arrPbLow = arrBot[0] = "+ (string) tfData.arrBot[0];
            drawPointStructure(tfData, -1, tfData.arrPbLow[0], tfData.arrPbLTime[0], MAJOR_STRUCTURE, false, enabledDraw);
   ...
   bool checkVolumeBreak(int type_break, MqlRates& barBreak, double value_swing, long vol_swing) {
      ...
      if (isTypeBreak == 1) { // Phá bằng râu nến
         ...
         if ( (type_break == 1 && double_bar_break > value_swing) || (type_break == -1 && double_bar_break < value_swing) ) {
            ...
            text += "; \nbodyBar("+(string)bodyBar+") - percent("+(string)percentIsDojiBar+")/100*highLowBar("+(string)highLowBar+") ("+(string)(highLowBar*percentIsDojiBar/100)+") = "+ (string)isDojiBar;
            if (isDojiBar < 0) {
